...
with graph.as_default():
    ...
    with sess.as_default():
        ...
        batches = inpH.batch_iter(list(zip(x1_test,x2_test,y_test,add_fea_test)), 2*FLAGS.batch_size, 1, shuffle=False)
        # Collect the predictions here
        all_predictions = []
        all_d=[]
        for db in batches:
            x1_dev_b,x2_dev_b,y_dev_b,add_fea_dev_b = zip(*db)
            batch_predictions, batch_acc, batch_sim = sess.run([predictions,accuracy,sim], {input_x1: x1_dev_b, input_x2: x2_dev_b, input_y:y_dev_b, 
                                                                                            add_fea:add_fea_dev_b, dropout_keep_prob: 1.0})
        ...
        print rank_all
        print 'MRR:' + str(sum(rank_all)/count)
