...
class InputHelper(object):
    ...
    def getDataSets(self, training_paths, dev_paths, max_document_length, percent_dev, batch_size):
        ...
        if dev_paths == None:
            ...
            x1 = np.asarray(list(vocab_processor.transform(x1_dev_text)))
            x2 = np.asarray(list(vocab_processor.transform(x2_dev_text)))

        sum_no_of_batches = sum_no_of_batches+(len(y_shuffled)//batch_size)
        train_set=(x1_shuffled,x2_shuffled,y_shuffled,add_fea_shuffled)
        dev_set=(x1,x2,x1_dev_text,x2_dev_text,y_dev,add_fea_dev)    
        gc.collect()
